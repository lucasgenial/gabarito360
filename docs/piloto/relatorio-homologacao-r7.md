...
| Imagens de aplicacao e Nginx | pendente de CI | daemon Docker local indisponivel |
| Ambiente sobe e health responde | pendente de CI | daemon Docker local indisponivel |
| Backup e restauracao isolada | pendente de CI | `scripts/infra/verify-restore.ps1` |
...
| CI da branch | pendente de push | GitHub Actions |
...
Este arquivo deve ser atualizado com resultados e links de CI antes de
...
transferidos para o job isolado da CI. Isso bloqueia o merge ate a CI passar,
mas nao invalida os gates locais independentes do daemon.
